...
from modules.modeling import TAB4IDC
...
def get_args(description='TAB4IDC on Captioning Task'):
...
def init_model(args, device, n_gpu, local_rank):
    ...
    model = TAB4IDC.from_pretrained(args.cross_model, args.decoder_model, cache_dir=cache_dir, state_dict=model_state_dict, task_config=args)
...
def train_epoch(epoch, args, model, train_dataloader, device, n_gpu, optimizer, scheduler, global_step, local_rank=0):
    ...
    for step, batch in enumerate(train_dataloader):
        ...
        if args.datatype == "clevr" or args.datatype == "open":
            input_ids, input_mask, segment_ids, bef_image, aft_image, no_image, gt_left_map, gt_right_map, nsc_map, image_mask, \
            pairs_input_caption_ids, pairs_decoder_mask, pairs_output_caption_ids, \
            no_pairs_input_caption_ids, no_pairs_decoder_mask, no_pairs_output_caption_ids = batch

            loss1, dec_loss, loc_loss = model(input_ids, segment_ids, input_mask, bef_image, aft_image, gt_left_map, gt_right_map, image_mask,
                         input_caption_ids=pairs_input_caption_ids, decoder_mask=pairs_decoder_mask,
                         output_caption_ids=pairs_output_caption_ids, datatype=args.datatype)
            loss2, dec_loss, loc_loss = model(input_ids, segment_ids, input_mask, bef_image, no_image, nsc_map, nsc_map, image_mask,
                         input_caption_ids=no_pairs_input_caption_ids, decoder_mask=no_pairs_decoder_mask,
                         output_caption_ids=no_pairs_output_caption_ids, datatype=args.datatype)
            loss = loss1 + loss2
        else:
            input_ids, input_mask, segment_ids, bef_image, aft_image, gt_left_map, gt_right_map, image_mask, \
            pairs_input_caption_ids, pairs_decoder_mask, pairs_output_caption_ids = batch

            loss, dec_loss, loc_loss = model(input_ids, segment_ids, input_mask, bef_image, aft_image, gt_left_map, gt_right_map, image_mask,
                         input_caption_ids=pairs_input_caption_ids, decoder_mask=pairs_decoder_mask,
                         output_caption_ids=pairs_output_caption_ids, datatype=args.datatype)
        ...
        if (step + 1) % args.gradient_accumulation_steps == 0:
            ...
            if global_step % log_step == 0 and local_rank == 0:
                logger.info("Epoch: %d/%s, Step: %d/%d, Lr: %s, Loss: %f, Time/step: %f, CCLoss: %f, DetLoss: %f", epoch + 1,
                            args.epochs, step + 1,
...
def greedy_decode(args, model, tokenizer, input_ids, segment_ids, input_mask, video, video_mask, gt_left_map, gt_right_map):
    _, visual_emb, _, visual_output, _, _ = model.get_sequence_visual_output(input_ids, segment_ids, input_mask, video,
...
def eval_epoch(args, model, test_dataloader, tokenizer, device, n_gpu, test_set=None, epoch=None):
    ...
    with torch.no_grad():
        for i, batch in enumerate(test_dataloader):
            ...
            if args.datatype == "clevr" or args.datatype == "open":
                input_ids, input_mask, segment_ids, bef_image, aft_image, nc_image, sc_target, nsc_target, gt_left_map, gt_right_map, nsc_map, image_mask, \
                pairs_input_caption_ids, pairs_decoder_mask, pairs_output_caption_ids, \
            ...
            result_list = greedy_decode(args, model, tokenizer, input_ids, segment_ids, input_mask, image_pair, image_mask, gt_left_map, gt_right_map)
            
            if args.datatype == "clevr" or args.datatype == "open":
                nc_result_list = greedy_decode(args, model, tokenizer, input_ids, segment_ids, input_mask, nc_image_pair, image_mask, nsc_map, nsc_map)
    ...
    if args.do_train:
        ...
        for epoch in range(resumed_epoch, args.epochs):
            ...
            if args.local_rank == 0:
                ...
                CIDEr = eval_epoch(args, model, val_dataloader, tokenizer, device, n_gpu, epoch=epoch)
                early_stop += 1
                if best_score <= CIDEr:
                    best_score = CIDEr
